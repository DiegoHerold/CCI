...
Motor inicial, puro e em memória para avaliar regras sobre um mapa de variáveis normalizadas. Será usado pelo `rule-service` para testes e pelo `rule-worker` para execução futura.
...
Variáveis ausentes geram `pending`, operador desconhecido gera `error`, comparações falsas geram `divergent` e comparações verdadeiras geram `approved`.
