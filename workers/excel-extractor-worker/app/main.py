...
from app.extractor import WORKER_VERSION, extract_excel
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="CCI Excel Extractor Worker",
        version=WORKER_VERSION,
        description="Internal worker API for template-based Excel extraction.",
    )

    @app.get("/health")
    async def health() -> dict[str, str]:
        return {"status": "ok", "service": "excel-extractor-worker"}

    @app.get("/ready")
    async def ready() -> dict[str, str]:
        return {"status": "ready", "service": "excel-extractor-worker"}
    ...
    async def extract(
        ...
        return ExtractResponse.model_validate(extract_excel(payload))
