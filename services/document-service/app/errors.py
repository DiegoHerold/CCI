...
class ClientContextError(ServiceError):
    def __init__(self, status_code: int, code: str, message: str) -> None:
...
class StorageUnavailableError(ServiceError):
    def __init__(self, message: str = "Document storage unavailable") -> None:
        super().__init__(503, "DOCUMENT_STORAGE_UNAVAILABLE", message)


class UpstreamUnavailableError(ServiceError):
    def __init__(self, service_name: str) -> None:
        super().__init__(503, f"{service_name.upper()}_UNAVAILABLE", f"{service_name} unavailable")
